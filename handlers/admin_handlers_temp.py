...
from aiogram.types import Message, CallbackQuery, ChatShared, ChatJoinRequest
...
from keyboards import (
    ...
    bot_settings_keyboard, channel_button_settings_keyboard
)
from states import (
...
#REMOVE_START
    ...
    views_count = get_total_views()
    today_users = get_today_users()
    today_views = get_today_views()
    weekly_users = get_weekly_users()
    ...
    # Grafik yasash
    graph = ""
    max_users = max([d['new_users'] for d in daily_stats]) if daily_stats else 1
    for day in reversed(daily_stats):
        bar_len = int((day['new_users'] / max(max_users, 1)) * 10)
...
+ Jami: <code>{users_count}</code>
+ Bugun: <code>{today_users}</code>
+ Haftalik: <code>{weekly_users}</code>
+ Faol (7 kun): <code>{active_users}</code>
└ 💎 Premium: <code>{premium_count}</code>

🎬 <b>Kinolar:</b> <code>{movies_count}</code>

👁 <b>Ko'rishlar:</b>
+ Jami: <code>{views_count}</code>
L Bugun: <code>{today_views}</code>

📺 <b>Kanallar:</b> <code>{channels_count}</code>
...
+ Jami: <code>{users_count}</code>
+ Bugun: <code>{today_users}</code>
L Haftalik: <code>{weekly_users}</code>
